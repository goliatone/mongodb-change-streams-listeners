...
/**
 * Simple SIG* manager, and more. The purpose of 
 * this class is to enable a way to clean up before
 * we exit. By clean up we mean notifying any connected
 * clients we are about to go down, persist any data
 * we might have in memory, etc. 
 * 
 * Obviously we want to minimize the amount of work we 
 * do when we intercept our singals.
 * 
...
class Signals extends EventEmitter {
    ...
    init(config = {}) {
        ...
        this.process.on('uncaughtException', this.uncaughtExceptionHandler.bind(this));
