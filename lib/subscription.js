...
class Subscription {
    ...
    fromJSON(src = {}) {
        //We should manage defaults 
        src = extend({}, this.defaultProperties, src);
    ...
    validate(obj) {
        if (!obj) obj = this.toJSON();

        if (typeof obj.collection !== 'string') return {
            success: false,
            error: 'Missing required property "collection"'
        };
    ...
    equals(subscription) {
        if (this.uuid && subscription.uuid) {
            return this.uuid === subscription.uuid;
        }

        if (this === subscription) return true;

        //We want to compare a subset of properties
        let a = {},
            b = {};

        ['data', 'collection',
            'pipeline', 'active', 'options'
        ].map(prop => {
            a[prop] = this[prop];
            b[prop] = subscription[prop];
        });
    ...
    addPipeline(operation) {
        ...
        Object.keys(operation).map(opKey => {
            if (supportedOperations.includes(opKey)) {
    ...
    getPipeline(optimize = true) {
         ...
         */
        if (optimize) {
            this.logger.info('Implement Subscription:optimize');
        }
    ...
    /**
     * Prepend a field name with the `fullDocument`
     * prefix in order to have a full path to the 
     * 
...
function _deepEqual(a, b) {
    ...
    for (prop in b) {
        propsInB += 1;
        if (!(prop in a) || !_deepEqual(a[prop], b[prop])) {
            return false;
        }
    }
